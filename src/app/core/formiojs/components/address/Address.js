      ...
      if (!this.builderMode) {
        if (this.component.map) {
          ...
          var _this$component = this.component,
              map = _this$component.map,
              providerOptions = _this$component.providerOptions;
          var key = map.key,
              region = map.region;

          if (key && !_lodash.default.get(providerOptions, 'params.key')) {
            _lodash.default.set(providerOptions, 'params.key', key);
          }

          if (region && !_lodash.default.get(providerOptions, 'params.region')) {
            _lodash.default.set(providerOptions, 'params.region', region);
          }
        }

        if (this.component.provider) {
          var _this$component2 = this.component,
              provider = _this$component2.provider,
              _providerOptions = _this$component2.providerOptions;
          this.provider = this.initializeProvider(provider, _providerOptions);
        }
      }
    }
  }, {
