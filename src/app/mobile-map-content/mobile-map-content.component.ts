...
@Component({
  selector: "app-mobile-map-content",
  templateUrl: "./mobile-map-content.component.html",
  styleUrls: ["./mobile-map-content.component.css"],
})
export class MobileMapContentComponent implements OnInit {
  coordinates: Coordinates;
  private geoCoder;
  issueForm = new FormGroup({
    name: new FormControl(),
    description: new FormControl(),
    myCurrentLocation: new FormControl(),
  ...
  public buton_type = [
    { button_name: "Refer a Needy Person", id: 1 },
    { button_name: "Sign up to Volunteer", id: 2 },
    { button_name: "Submit Local Information", id: 3 },
  ];
  @ViewChild("closeModal") private closeModal: ElementRef;
  @ViewChild("content") private contentmodalRef: TemplateRef<Object>;
  @ViewChild("location_details") private location_detailsRef: TemplateRef<Object>;
  @ViewChild("dynamic_form") dynamic_form: ElementRef;
  ...
  fields: FormlyFieldConfig[] = [
    ...
    },
    {
      ...
      type: "input",
      templateOptions: {
        label: "Which town / city are you based in?",
        placeholder: "",
      },
    },
  ];
  ...
  model_needy = {};
  fields_needy: FormlyFieldConfig[] = [
    {
      key: "input",
      type: "input",
      templateOptions: {
  ...
  model_local = {};
  fields_local: FormlyFieldConfig[] = [
    {
      key: "input",
      type: "input",
      templateOptions: {
  ...
  getcurLocation() {
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition((pos) => {
        ...
        this.issueForm.controls.myCurrentLocation.setValue(
          this.myCurrentLocation
        );
  ...
  openGoogelMapsModal() {
    ...
    modalRef.result.then(
      (result) => {
        ...
        this.allow_location(this.coordinates);
  ...
  item_post() {
    ...
    this.dataService.postitemdata(payload).subscribe((data) => { });
  }
  ...
  open_tree(content) {
    this.modalService
      .open(content, {
        ...
        windowClass: "modal_customize_tree",
      })
      .result.then(
        (result) => {
        },
  ...
  SelectCity(event) {
    this.open_tree(this.contentRef);
  ...
  ngOnInit() {
    this.menuService.selectedMenu$.subscribe((menukey) => {
      this.get_form_type_list();
      if (menukey == "add") {
        this.modalService
          .open(this.contentmodalRef, {
            ariaLabelledBy: "modal-basic-title",
    ...
    this.dataService.getnewMenuList(obj).subscribe((data) => {
  ...
  onSubmitTemplate() {
    ...
    this.modalService.open(this.location_detailsRef, {
      ariaLabelledBy: "modal-basic-title",
      windowClass: "modal_customize_service_top",
    });
  }

  allow_location(cordinate_values) {
    let selected_location =
      cordinate_values.latitude + "," + cordinate_values.longitude;
    let payload = {
      form_data: this.previewForm.value,
      myCurrentLocation: selected_location,
      submissionTypeId: this.selectedValue,
    };
    console.log(payload, "outgoing data");

    this.dataService.submissionForm(payload).subscribe((formtypelist) => {
  ...
  open_needy_person(refer_needy, form_arr) {
  ...
  preview_form(form_arr) {
    this.typeId = "";
    this.typeId = form_arr.typeId;
    this.open_needy_person(this.refer_needyRef, form_arr);
  }
  ...
  open_form(form_id) {
    ...
    } else if (form_id.form_id == "2") {
      this.refer_needy_person(this.signup_volunteerref);
    } else if (form_id == "3") {
      this.local_information(this.local_informationref);
    }
  }
  ...
  markerDrag(event) {
    ...
    this.myCurrentLocation = "" + this.lng + ", " + this.lat;
  }

  img_change(e) {
    this.img_upload_data = e.data.upload;
  }
  ...
  getAddress(latitude, longitude): any {
    this.geoCoder.geocode(
      ...
      (results, status) => {
        ...
        var temadd = this.address;
        console.log(this.address, "selected lat and longitude address");
        return temadd;
      }
    );
  }
  ...
  getlocationApi(e) {
    ...
    if (temp1[3] === "custom_btn_css") {
      ...
      this.dataService.getorguserdata(payload).subscribe((formtypelist) => { });
    }
  }
}
